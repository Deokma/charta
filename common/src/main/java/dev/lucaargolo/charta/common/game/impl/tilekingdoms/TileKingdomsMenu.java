...
public class TileKingdomsMenu extends AbstractCardMenu<TileKingdomsGame, TileKingdomsMenu> {
    ...
    private final ContainerData data = new ContainerData() {
        @Override public int get(int index) {
            ...
            if (index == OFF_CLAIMC) return Math.min(g.claimsSnapshot != null ? g.claimsSnapshot.length : 0, MAX_CLAIMS);
    ...
    public TileKingdomsBoardPayload pollBoardPayload() {
        ...
        if (g.boardDirty || (g.claimsSnapshot != null)) {
            boolean dirty = g.boardDirty;
            g.boardDirty = false;
            short[] grid = g.boardSnapshot != null ? g.boardSnapshot : new short[TileKingdomsBoard.SIZE * TileKingdomsBoard.SIZE];
            int[]   claims = g.claimsSnapshot != null ? g.claimsSnapshot.clone() : new int[0];
            if (dirty) return new TileKingdomsBoardPayload(containerId, grid, claims);
        }
        return null;
    }
    ...
    // ── Client accessors ──────────────────────────────────────────────────────
    public short[] getBoardGrid()     { return clientBoardGrid; }
    ...
    public TileType getCurrentTile()   { int v = data.get(OFF_TILE); return (v >= 0 && v < TileType.values().length) ? TileType.values()[v] : null; }
